...
const DataVisualizationModule = (() => {
  ...
  const updateSummaryStats = (data) => {
    ...
    const avgCalories = Math.round(data.calories.reduce((sum, val) => sum + val, 0) / data.calories.length);
    ...
    document.getElementById('weightChange').textContent = `${weightChange.toFixed(1)} kg`;
    ...
    const avgPace = Math.round(data.pace.reduce((sum, val) => sum + val, 0) / data.pace.length);
    const avgHeartRate = Math.round(data.heartRate.reduce((sum, val) => sum + val, 0) / data.heartRate.length);
    
    document.getElementById('totalDistance').textContent = `${totalDistance.toFixed(1)} km`;
    document.getElementById('avgPace').textContent = formatPace(avgPace);
    document.getElementById('totalTime').textContent = `${(totalDistance * avgPace / 60).toFixed(1)} hrs`;
