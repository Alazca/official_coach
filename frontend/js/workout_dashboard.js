...
const WorkoutDashboard = (() => {
  ...
  const initialize = () => {
    document.addEventListener('DOMContentLoaded', () => {
      ...
      if (typeof CalendarModule !== 'undefined' && CalendarModule.initializeCalendar) {
      ...
      if (WorkoutModule.loadWorkout) {
          ...
          console.error("WorkoutModule.loadWorkout is not available for initial load.");
      }
    ...
    if (resetBtn) {
      resetBtn.addEventListener('click', () => {
        if (WorkoutModule.resetDayWorkout) {
          ...
          if (confirm("Are you sure you want to clear the workout for this day? Unsaved changes will be lost.")) {
    ...
    if (workoutTypeBtns.length > 0) {
        workoutTypeBtns.forEach(btn => {
            ...
            if (workoutType) {
                ...
                console.warn('Workout type button found without data-workout-type attribute:', btn);
            }
        });
    } else {
