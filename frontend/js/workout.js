...
const WorkoutModule = (() => {
  ...
  const recommendationsLoaderEl = document.getElementById(
  ...
  const loadWorkout = async (dateString) => {
    ...
    currentWorkout = {
      // Reset current workout before loading
      date: dateString,
      exercises: [],
      notes: "",
      duration: 0,
    ...
    try {
      ...
      const data = await apiRequest(
        `${config.loadWorkoutEndpoint}?date=${dateString}`,
        "GET",
      );
      if (data && data.workout) {
        ...
        currentWorkout.estimated_duration =
          data.workout.estimated_duration || 0;
        currentWorkout.calories_burn_estimate =
          data.workout.calories_burn_estimate || 0;
        currentWorkout.workout_notes_from_engine =
          data.workout.workout_notes_from_engine || [];
  ...
  const getCurrentWorkout = () => {
    ...
    currentWorkout.notes = workoutNotesEl
      ? workoutNotesEl.value
      : currentWorkout.notes;
    currentWorkout.duration = workoutDurationEl
      ? parseInt(workoutDurationEl.value, 10) || 0
  ...
  const generateWorkout = async (workoutType) => {
    ...
    if (workoutListEl)
      workoutListEl.innerHTML =
        '<p class="text-center text-gray-400">Generating workout...</p>';
    ...
    try {
      ...
      const data = await apiRequest(
        config.generateWorkoutEndpoint,
        "POST",
        requestData,
      );
      ...
      if (data && data.workout_plan) {
        ...
        console.error(
          "Failed to generate workout, response format incorrect:",
          data,
        );
  ...
  const getRecommendations = async () => {
    ...
    if (recommendationsLoaderEl)
      recommendationsLoaderEl.classList.remove("hidden");
    ...
    try {
      ...
      if (data && data.recommended_exercises) {
        // Assuming backend returns { recommended_exercises: { dimension: [ex1, ex2] } }
        displayRecommendations(data.recommended_exercises);
      } else {
        if (recommendationsListEl)
          recommendationsListEl.innerHTML =
            '<p class="text-gray-400">No recommendations available at this time.</p>';
      }
    } catch (error) {
      console.error("Failed to get recommendations:", error);
      if (recommendationsListEl)
        recommendationsListEl.innerHTML = `<p class="text-red-400">Error loading recommendations: ${error.message}</p>`;
    } finally {
      if (recommendationsLoaderEl)
        recommendationsLoaderEl.classList.add("hidden");
    }
  };
  ...
  const displayRecommendations = (recommendations) => {
    ...
    if (Object.keys(recommendations).length === 0) {
      recommendationsListEl.innerHTML =
        '<p class="text-gray-400">No specific recommendations based on current analysis.</p>';
      return;
    }
  ...
  const saveWorkout = async () => {
    ...
    if (workoutData.exercises.length === 0) {
      alert(
        "Please add at least one exercise or generate a workout before saving.",
      );
      return;
    }
  ...
  const displayWorkout = () => {
    ...
    if (
      ...
      console.error(
        "One or more UI elements not found for displaying workout.",
      );
      return;
    }
    ...
    selectedDateEl.textContent = new Date(
      currentWorkout.date + "T00:00:00",
    ...
    if (currentWorkout.exercises.length === 0) {
      workoutListEl.innerHTML =
        '<p class="text-center text-gray-400">No exercises added yet. Generate a workout or add exercises manually.</p>';
    } else {
      currentWorkout.exercises.forEach((exercise, index) => {
        const exerciseEl = document.createElement("div");
        exerciseEl.className =
          "bg-gray-800 p-4 rounded-lg border border-gray-700 flex justify-between items-center";
