...
const CoachUtils = (() => {
     ...
     */
    const sendPostAndRedirect = async (endpoint, data, redirectUrl, onSuccess = null, onError = null) => {
        try {
            // Show loading indicator
            const loadingIndicator = document.createElement('div');
            loadingIndicator.className = 'fixed top-0 left-0 w-full h-1 bg-red-600 animate-pulse z-50';
            ...
            const indicator = document.querySelector('.fixed.top-0.left-0.w-full.h-1');
            if (indicator) {
     ...
     * @param {string} redirectUrl - URL to redirect to after successful request
     */
    const setupButtonWithPostAndRedirect = (buttonId, endpoint, data, redirectUrl) => {
