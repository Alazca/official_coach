...
def insert_workout(conn, workout_data):
    """
    Insert a new workout record into the workouts table.

    Args:
        conn: Database connection object
        workout_data: Dictionary containing workout information with keys:
    ...
    Returns:
        Integer: ID of the newly inserted workout
    ...
    try:
        ...
        query = """
            INSERT INTO workouts (name, date, notes, duration)
            VALUES (:name, :date, :notes, :duration)
        """

        # Execute query with parameter binding for security
        cursor.execute(query, workout_data)
...
def insert_workout_sets(conn, sets_data):
    """
    Insert multiple workout sets into the workout_sets table.

    Args:
        conn: Database connection object
        sets_data: List of dictionaries, each containing:
            - workout_id: ID of the parent workout
            - exercise_name: Name of the exercise
            - reps: Number of repetitions
            - weight: Weight used (optional)
            - set_number: Position in sequence (optional)
            - notes: Additional notes (optional)

    Returns:
        Integer: Count of successfully inserted sets
    ...
    # Track number of successful insertions
    inserted_count = 0

    try:
        # Begin a transaction for better performance with multiple inserts
        conn.execute("BEGIN TRANSACTION")

        # Prepare query with named placeholders
        query = """
            INSERT INTO workout_sets 
            (workout_id, exercise_name, reps, weight, set_number, notes)
            VALUES 
            (:workout_id, :exercise_name, :reps, :weight, :set_number, :notes)
        """

        # Process each set in the list
