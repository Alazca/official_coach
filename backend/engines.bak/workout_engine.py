...
def generate_workout(user_data, user_id):
    ...
    try:
        ...
        fitness_level = user_data.get(
            "fitness_level", "intermediate"
        )  # beginner, intermediate, advanced
        available_equipment = user_data.get(
            "available_equipment", ["bodyweight"]
        )  # list of available equipment
        target_muscle_groups = user_data.get(
            "target_muscle_groups", ["full_body"]
        )  # list of muscle groups
        workout_duration = user_data.get(
            "workout_duration", 45
        )  # desired workout length in minutes
        workout_goal = user_data.get(
            "workout_goal", "general_fitness"
        )  # strength, hypertrophy, endurance, etc.
...
def build_workout_plan(
    fitness_level, equipment, muscle_groups, duration, goal, intensity, recovery_status
):
    """
    ...
    if "full_body" in muscle_groups:
        ...
        workout_plan = generate_split_workout(
            muscle_groups, equipment, fitness_level, goal, intensity, recovery_status
        )
...
def generate_split_workout(
    ...
    if goal == "strength":
        for exercise in workout:
            if (
                "reps" in exercise
                and isinstance(exercise["reps"], str)
                and "-" in exercise["reps"]
            ):
...
def adjust_workout_length(workout_plan, target_duration):
    ...
    if current_duration > target_duration:
        ...
        estimated_duration = (
            sum(
                [
                    ex["sets"] * 45 + ex["rest"] * (ex["sets"] - 1)
                    for ex in adjusted_workout
                ]
            )
            / 60
        )
        ...
        for exercise in isolation_exercises:
            ...
            exercise_duration = (
                exercise["sets"] * 45 + exercise["rest"] * (exercise["sets"] - 1)
            ) / 60
...
def estimate_calories(workout_plan, user_id):
    ...
    intensity = calculate_intensity_level(
    ...
    weight_training_exercises = sum(
        1 for ex in workout_plan if "cardio" not in ex.get("muscle_group", "")
    )
...
def generate_workout_notes(workout_plan, rest_needs, user_id):
    ...
    muscles_needing_rest = [
        muscle
        for muscle in working_muscle_groups
        if muscle in rest_needs and rest_needs[muscle] == "needs_rest"
    ]

    if muscles_needing_rest:
        notes.append(
            f"Be cautious with {', '.join(muscles_needing_rest)} exercises as these muscle groups may need more recovery."
        )
    ...
    if intensity == "intense" or intensity == "very intense":
        notes.append(
            "This is a high-intensity workout. Listen to your body and adjust weights if needed."
        )
    ...
    for exercise in workout_plan:
        if "Deadlift" in exercise["exercise"]:
            notes.append(
                "Focus on proper form during deadlifts to protect your lower back."
            )
        elif "Squat" in exercise["exercise"]:
            notes.append(
                "Keep your chest up and knees tracking over toes during squats."
            )
