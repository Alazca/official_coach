...
def evaluate_vectors(user_input, user_id):
    ...
    result = {"readiness_score": 0, "recommendations": []}
    ...
    try:
        ...
        result["recommendations"] = [
            "Unable to generate specific recommendations due to an error."
        ]
        return result
...
def generate_recommendations(readiness_score, user_input):
    ...
    if readiness_score >= 80:
        recommendations.append(
            "High readiness: You're well-recovered and ready for a challenging workout."
        )
        recommendations.append(
            "Consider high-intensity or heavy strength training today."
        )

    elif readiness_score >= 60:
        recommendations.append(
            "Good readiness: You're in good recovery state for moderate training."
        )
        recommendations.append(
            "Moderate intensity workouts or technical skill work recommended."
        )

    elif readiness_score >= 40:
        recommendations.append(
            "Moderate readiness: Some fatigue present, approach training cautiously."
        )
        recommendations.append(
            "Consider light to moderate training with focus on technique."
        )

    else:
        recommendations.append(
            "Low readiness: Recovery is priority. Your body needs rest."
        )
    ...
    if sleep < 4:
        recommendations.append(
            "Poor sleep detected. Consider sleep hygiene improvements and limit high-intensity exercise."
        )

    stress = user_input.get("stress_level", 5)
    if stress > 7:
        recommendations.append(
            "High stress detected. Stress-reducing activities like yoga or light cardio may be beneficial."
        )

    soreness = user_input.get("soreness_level", 5)
    if soreness > 7:
        recommendations.append(
            "High muscle soreness detected. Focus on recovery modalities like stretching, foam rolling, or light movement."
        )
...
def prepare_visualization_data(
    strength_evaluation: Dict[str, Any], conditioning_evaluation: Dict[str, Any]
) -> Dict[str, Any]:
    """
