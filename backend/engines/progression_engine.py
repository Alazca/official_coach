    ...
    try:
        ...
        for workout_type, volumes in volumes_by_type.items():
            ...
            if past_avg > 0:
                ...
                progression_score = 50 + (percent_change * 2.5)  # 10% increase = 75 score
...
def calculate_consistency_score(user_id: int, time_frame: str = "month") -> float:
    ...
    try:
        ...
        planned_days_per_week = result['planned_workout_days'] if result else 3  # Default to 3 days/week
...
def calculate_endurance_progression(user_id:int, time_frame: str = "month"):
    ...
    try:
        ...
        cardio_workouts = [w for w in workout_history if w.get('workout_type', '').lower() in 
        ...
        if all(durations):
            ...
            duration_change = ((current_duration - initial_duration) / initial_duration * 100) if initial_duration > 0 else 0
        ...
        if all(distances):
            ...
            distance_change = ((current_distance - initial_distance) / initial_distance * 100) if initial_distance > 0 else 0
        ...
        if all(avg_heart_rates):
            ...
            hr_change = ((initial_hr - current_hr) / initial_hr * 100) if initial_hr > 0 else 0
...
def generate_progression_insights(workout_history, volume_progression, intensity_progression, consistency_score):
    """
    ...
    if volume_progression >= 70:
        insights.append("Your workout volume is steadily increasing, showing good progress in training capacity.")
    elif volume_progression <= 30:
        insights.append("Your workout volume has been decreasing, which may indicate fatigue or insufficient recovery.")
    else:
        insights.append("Your workout volume is relatively stable.")
    
    # Intensity insights
    if intensity_progression >= 70:
        insights.append("Your workout intensity is trending upward, indicating improved strength and conditioning.")
    elif intensity_progression <= 30:
        insights.append("Your workout intensity has been decreasing, which may require attention to training stimulus.")
    else:
        insights.append("Your workout intensity is being maintained at a consistent level.")
    
    # Consistency insights
    if consistency_score >= 80:
        insights.append("Excellent workout consistency! Your adherence to your training schedule is a key factor in your results.")
    elif consistency_score >= 60:
        insights.append("Good workout consistency overall, with room for minor improvements in schedule adherence.")
    elif consistency_score >= 40:
        insights.append("Moderate workout consistency. Try to improve adherence to your planned schedule for better results.")
    else:
        insights.append("Your workout consistency needs improvement. Consider addressing schedule barriers.")
    ...
    dominant_type = max(workout_types.items(), key=lambda x: x[1])[0] if workout_types else None
    if dominant_type:
        insights.append(f"Your training is primarily focused on {dominant_type} workouts.")
    
    # Check for balance
    if len(workout_types) == 1 and workout_types:
        insights.append("Your training lacks variety. Consider incorporating different workout types for more balanced results.")
...
def generate_progression_recommendations(progression_score, workout_history):
    ...
    if progression_score >= 80:
        recommendations.append("Your progression is excellent. Consider setting more challenging goals to continue advancement.")
        recommendations.append("Try incorporating advanced techniques like periodization to further optimize progress.")
    
    elif progression_score >= 60:
        recommendations.append("Your progression is good. Focus on consistency and gradual intensity increases.")
        recommendations.append("Consider tracking additional performance metrics to identify specific areas for improvement.")
    
    elif progression_score >= 40:
        recommendations.append("Your progression is moderate. Review your training program for potential optimization.")
        recommendations.append("Focus on progressive overload by gradually increasing volume or intensity each week.")
    
    else:
        recommendations.append("Your progression needs attention. Consider revising your training approach.")
        recommendations.append("Focus first on consistency, then gradually build volume before increasing intensity.")
    ...
    if workout_history and len(workout_history) >= 4:
        ...
        volume_stagnant = all(abs(v - recent_volumes[0]) < recent_volumes[0] * 0.05 for v in recent_volumes) if all(recent_volumes) else False
        intensity_stagnant = all(abs(i - recent_intensities[0]) < 5 for i in recent_intensities) if all(recent_intensities) else False
        
        if volume_stagnant and intensity_stagnant:
            recommendations.append("You appear to be plateauing. Consider introducing variation through new exercises or training methods.")
            recommendations.append("A deload week followed by a change in program may help break through your plateau.")
    
    # Check workout frequency
    workout_dates = [w.get('workout_date') for w in workout_history]
    if workout_dates and len(workout_dates) >= 2:
        date_diffs = [(workout_dates[i-1] - workout_dates[i]).days for i in range(1, len(workout_dates))]
        avg_days_between = sum(date_diffs) / len(date_diffs) if date_diffs else 0
        
        if avg_days_between > 4:
            recommendations.append("Your workout frequency is low. Consider increasing training frequency for better results.")
        elif avg_days_between < 1.5:
            recommendations.append("Your workout frequency is very high. Ensure you're getting adequate recovery between sessions.")
...
def suggest_progression_plan(user_id, target_goal):
    ...
    try:
        ...
        return {
            ...
            "duration_weeks": 4,
            "phases": [{"name": "Building consistency", "focus": "Establishing workout routine"}],
...
def create_strength_phases(fitness_level):
    ...
    if fitness_level == "beginner":
        return [
            {"name": "Foundation", "duration_weeks": 2, "focus": "Movement patterns and technique"},
            {"name": "Base strength", "duration_weeks": 3, "focus": "Progressive loading with moderate weights"},
            {"name": "Strength development", "duration_weeks": 3, "focus": "Increasing intensity with adequate recovery"}
        ]
    elif fitness_level == "intermediate":
        return [
            {"name": "Hypertrophy block", "duration_weeks": 3, "focus": "Volume accumulation to build muscle"},
            {"name": "Strength block", "duration_weeks": 3, "focus": "Increasing intensity with moderate volume"},
            {"name": "Peak strength", "duration_weeks": 2, "focus": "High intensity, lower volume for maximal strength"}
        ]
    else:  # advanced
        return [
            {"name": "Volume accumulation", "duration_weeks": 2, "focus": "High volume to drive adaptation"},
            {"name": "Strength accumulation", "duration_weeks": 2, "focus": "Progressive loading with moderate volume"},
            {"name": "Intensity phase", "duration_weeks": 2, "focus": "High intensity work to maximize strength"},
            {"name": "Peak/taper", "duration_weeks": 2, "focus": "Reduced volume, maintained intensity for peaking"}
        ]
...
def create_hypertrophy_phases(fitness_level):
    ...
    if fitness_level == "beginner":
        return [
            {"name": "Movement patterns", "duration_weeks": 2, "focus": "Learning exercises and technique"},
            {"name": "Base volume", "duration_weeks": 3, "focus": "Building workload capacity"},
            {"name": "Hypertrophy development", "duration_weeks": 3, "focus": "Increased volume in hypertrophy ranges"}
        ]
    elif fitness_level == "intermediate":
        return [
            {"name": "Metabolic phase", "duration_weeks": 2, "focus": "Higher reps, shorter rest periods"},
            {"name": "Volume phase", "duration_weeks": 3, "focus": "Moderate reps with increased sets"},
            {"name": "Tension phase", "duration_weeks": 3, "focus": "Focus on time under tension and mind-muscle connection"}
        ]
    else:  # advanced
        return [
            {"name": "Primer phase", "duration_weeks": 1, "focus": "Preparing muscles for growth stimulus"},
            {"name": "Volume block", "duration_weeks": 3, "focus": "Accumulating volume across varying rep ranges"},
            {"name": "Intensification", "duration_weeks": 2, "focus": "Increased load with maintained volume"},
            {"name": "Specialization", "duration_weeks": 2, "focus": "Focus on lagging muscle groups"}
        ]
...
def create_endurance_phases(fitness_level):
    ...
    if fitness_level == "beginner":
        return [
            {"name": "Base building", "duration_weeks": 3, "focus": "Building aerobic capacity"},
            {"name": "Endurance development", "duration_weeks": 3, "focus": "Increasing duration of steady state work"},
            {"name": "Mixed endurance", "duration_weeks": 2, "focus": "Introducing intensity variations"}
        ]
    elif fitness_level == "intermediate":
        return [
            {"name": "Aerobic base", "duration_weeks": 2, "focus": "Building aerobic efficiency"},
            {"name": "Threshold work", "duration_weeks": 3, "focus": "Improving lactate threshold"},
            {"name": "Mixed zones", "duration_weeks": 3, "focus": "Training across different heart rate zones"}
        ]
    else:  # advanced
        return [
            {"name": "Aerobic accumulation", "duration_weeks": 2, "focus": "Building aerobic capacity"},
            {"name": "Threshold development", "duration_weeks": 2, "focus": "Improving lactate threshold and clearance"},
            {"name": "VO2 max development", "duration_weeks": 2, "focus": "High intensity intervals for maximal oxygen uptake"},
            {"name": "Specific endurance", "duration_weeks": 2, "focus": "Sport-specific endurance patterns"}
        ]
...
def create_fat_loss_phases(fitness_level):
    ...
    if fitness_level == "beginner":
        return [
            {"name": "Movement foundation", "duration_weeks": 2, "focus": "Building movement capacity and consistency"},
            {"name": "Volume introduction", "duration_weeks": 3, "focus": "Gradually increasing workout density"},
            {"name": "Metabolic conditioning", "duration_weeks": 3, "focus": "Increasing caloric expenditure through conditioning"}
        ]
    elif fitness_level == "intermediate":
        return [
            {"name": "Base building", "duration_weeks": 2, "focus": "Building work capacity and consistency"},
            {"name": "Metabolic phase", "duration_weeks": 3, "focus": "HIIT and circuit training for caloric burn"},
            {"name": "Strength maintenance", "duration_weeks": 3, "focus": "Preserving muscle while emphasizing fat loss"}
        ]
    else:  # advanced
        return [
            {"name": "Metabolic primer", "duration_weeks": 1, "focus": "Preparing for increased workload"},
            {"name": "High volume phase", "duration_weeks": 2, "focus": "Maximum caloric expenditure through volume"},
            {"name": "Intensity phase", "duration_weeks": 2, "focus": "HIIT and metabolic resistance training"},
            {"name": "Recovery conditioning", "duration_weeks": 1, "focus": "Active recovery while maintaining deficit"},
            {"name": "Peak intensity", "duration_weeks": 2, "focus": "Maximum intensity methods for final results"}
        ]
...
def generate_weekly_targets(goal, fitness_level, duration_weeks):
    ...
    for week in range(1, duration_weeks + 1):
        ...
        if goal == "strength":
            if week == 1:
                weekly_targets.append(f"Week {week}: Establish baseline 1RMs for key lifts")
            else:
                percentage = round((cumulative_progression - 1) * 100, 1)
                weekly_targets.append(f"Week {week}: Target {percentage}% increase in working weights from baseline")
                
        elif goal == "hypertrophy":
            if week == 1:
                weekly_targets.append(f"Week {week}: Establish baseline training volume")
            else:
                if week % 4 == 0:
                    weekly_targets.append(f"Week {week}: Deload week - reduce volume by 40%, maintain intensity")
                else:
                    percentage = round((cumulative_progression - 1) * 100, 1)
                    weekly_targets.append(f"Week {week}: Increase training volume by {percentage}% from baseline")
                
        elif goal == "endurance":
            if week == 1:
                weekly_targets.append(f"Week {week}: Establish baseline distance and duration")
            else:
                percentage = round((cumulative_progression - 1) * 100, 1)
                weekly_targets.append(f"Week {week}: Increase training distance by {percentage}% from baseline")
                
        elif goal == "fat_loss":
            if week == 1:
                weekly_targets.append(f"Week {week}: Establish baseline workout density (work completed per minute)")
            else:
                percentage = round((cumulative_progression - 1) * 100, 1)
