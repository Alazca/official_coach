...
def weighted_similarity(vec1: np.ndarray, vec2: np.ndarray, weights: np.ndarray = None) -> float:
    """
...
def load_target_profile(profile_name: str) -> List[float]:
    """
    Load a target profile vector from a JSON file.
    
    Parameters:
        profile_name (str): Name of the profile JSON file
        
    Returns:
        list: Vector representation of the target profile
    """
    # Determine the base directory for profiles
    base_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'profiles')
    ...
    try:
        ...
        # Extract the vector from the profile data
        if isinstance(profile_data, dict) and 'vector' in profile_data:
            return profile_data['vector']
        elif isinstance(profile_data, list):
            return profile_data
        else:
            raise ValueError(f"Invalid profile format in {profile_name}")
    
    except FileNotFoundError:
        raise FileNotFoundError(f"Profile {profile_name} not found at {profile_path}")
    except json.JSONDecodeError:
        raise ValueError(f"Invalid JSON in profile {profile_name}")
...
def vector_distance(vec1: np.ndarray, vec2: np.ndarray, method: str = 'euclidean') -> float:
    """
...
def vector_to_percentile(vec: np.ndarray, reference_matrix: np.ndarray) -> np.ndarray:
    ...
    for i in range(len(vec)):
        ...
        percentiles[i] = np.sum(reference_values <= vec[i]) / len(reference_values) * 100
...
def create_weighted_profile(base_profile: List[float], 
                           adjustments: Dict[str, float], 
                           dimension_labels: List[str]) -> List[float]:
    """
    Create a personalized profile by adjusting a base profile with weights.
    
    Parameters:
        base_profile (List[float]): Base vector to adjust
        adjustments (Dict[str, float]): Adjustments by dimension name
        dimension_labels (List[str]): Names for each vector dimension
        
    Returns:
        List[float]: Adjusted profile vector
    """
    adjusted_profile = np.array(base_profile, dtype=float)
    
    for dim_name, adjustment in adjustments.items():
        try:
            idx = dimension_labels.index(dim_name)
            adjusted_profile[idx] *= adjustment
        except ValueError:
            # Ignore adjustments for dimensions that don't exist
            pass
    
    return adjusted_profile.tolist()
...
def save_profile(profile_data: Union[List[float], Dict[str, Any]], 
                profile_name: str) -> str:
    """
    Save a profile vector to a JSON file.
    
    Parameters:
        profile_data (Union[List[float], Dict[str, Any]]): Profile vector or dictionary
...
def aggregate_vectors(vectors: List[np.ndarray], 
