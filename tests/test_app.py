...
def test_checkin_success(client):
    ...
    checkin_resp = client.post('/api/checkin',
...
def test_get_checkins_success(client):
    ...
    # Now fetch check-ins
    history_resp = client.get(f"/api/checkins/{user_id}", headers={
        "Authorization": f"Bearer {token}"
    })
...
def test_get_nutrition_success(client):
    ...
    # Call the route
    response = client.get('/api/nutrition', headers={
        'Authorization': f'Bearer {token}'
    })
...
def test_get_workout_history_success(client):
    ...
    response = client.get('/api/workout-history', headers={
        'Authorization': f'Bearer {token}'
    })
